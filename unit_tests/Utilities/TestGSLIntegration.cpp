...
double quadratic_func(double x, void* /* params */) {
    return x * x;
}

// Test function: f(x) = sin(x)
double sin_func(double x, void* /* params */) {
    return std::sin(x);
}
...
TEST_F(GSLIntegrationTest, LinearFunction) {
    ...
    double result, error;
    int status = gsl_integration_qag(&F, 0.0, 1.0, 1e-10, 1e-10, 1000,
...
TEST_F(GSLIntegrationTest, QuadraticFunction) {
    ...
    double result, error;
    int status = gsl_integration_qag(&F, 0.0, 2.0, 1e-10, 1e-10, 1000,
                                     GSL_INTEG_GAUSS15, w, &result, &error);
...
TEST_F(GSLIntegrationTest, SinFunction) {
    ...
    double result, error;
    int status = gsl_integration_qag(&F, 0.0, M_PI, 1e-10, 1e-10, 1000,
                                     GSL_INTEG_GAUSS15, w, &result, &error);
...
TEST_F(GSLIntegrationTest, NegativeRange) {
    ...
    double result, error;
    int status = gsl_integration_qag(&F, -1.0, 1.0, 1e-10, 1e-10, 1000,
                                     GSL_INTEG_GAUSS15, w, &result, &error);
...
TEST_F(GSLIntegrationTest, SmallRange) {
    ...
    double result, error;
    int status = gsl_integration_qag(&F, 0.0, 0.1, 1e-10, 1e-10, 1000,
                                     GSL_INTEG_GAUSS15, w, &result, &error);
