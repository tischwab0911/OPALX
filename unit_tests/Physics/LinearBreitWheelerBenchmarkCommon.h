...
inline double sampledObservable(const Physics::LinearBreitWheeler::SampledEvent& event,
                                FinalState state,
                                Observable observable) {
    if (observable == Observable::Theta) {
        return state == FinalState::Positron
            ? polarAngleRad(event.positronMomentumLabGeV)
 ...
 */
inline Histogram sampleHistogram(const HistogramConfig& config,
                                 FinalState state,
                                 Observable observable,
    ...
    const double laserPhotonEnergyGeV = Physics::LinearBreitWheeler::photonEnergyFromWavelengthGeV(
        config.wavelength_m);
    const auto kernel = Physics::LinearBreitWheeler::makeSamplingKernel(config.highEnergyPhotonEnergyGeV,
                                                                        laserPhotonEnergyGeV,
                                                                        config.highEnergyDirection,
...
inline void writeHistogramCSV(const Histogram& histogram,
                              const std::filesystem::path& outputPath) {
    std::ofstream output(outputPath);
    if (!output) {
        throw std::runtime_error("LinearBreitWheelerBenchmark: unable to open output file "
                                 + outputPath.string());
    }
    output << "# center,density,count\n";
    output.precision(17);
    for (std::size_t i = 0; i < histogram.centers.size(); ++i) {
        output << histogram.centers[i] << ','
               << histogram.density[i] << ','
               << histogram.counts[i] << '\n';
    }
}

inline void writeJointHistogramCSV(const JointHistogram& histogram,
                                   const std::filesystem::path& outputPath) {
    std::ofstream output(outputPath);
    if (!output) {
        throw std::runtime_error("LinearBreitWheelerBenchmark: unable to open output file "
    ...
    for (std::size_t i = 0; i < histogram.energyCentersGeV.size(); ++i) {
        for (std::size_t j = 0; j < histogram.thetaCentersRad.size(); ++j) {
            output << histogram.energyCentersGeV[i] << ','
...
inline Histogram readHistogramCSV(const std::filesystem::path& inputPath) {
    ...
    if (!input) {
        throw std::runtime_error("LinearBreitWheelerBenchmark: unable to open input file "
                                 + inputPath.string());
    }
...
inline JointHistogram readJointHistogramCSV(const std::filesystem::path& inputPath) {
    ...
    while (std::getline(input, line)) {
        ...
        rows.push_back(Row{values[0], values[1], values[2], values.size() >= 4 ? values[3] : 0.0});
        if (histogram.energyCentersGeV.empty() || histogram.energyCentersGeV.back() != values[0]) {
    ...
    histogram.densityPerGeVRad.assign(histogram.energyCentersGeV.size(),
                                      std::vector<double>(histogram.thetaCentersRad.size(), 0.0));
    histogram.counts.assign(histogram.energyCentersGeV.size(),
                            std::vector<double>(histogram.thetaCentersRad.size(), 0.0));
    for (const auto& row : rows) {
        std::size_t i = 0;
        while (i < histogram.energyCentersGeV.size() && histogram.energyCentersGeV[i] != row.energyCenter) {
            ++i;
        }
        std::size_t j = 0;
        while (j < histogram.thetaCentersRad.size() && histogram.thetaCentersRad[j] != row.thetaCenter) {
            ++j;
        }
    ...
    if (histogram.energyCentersGeV.size() >= 2) {
        histogram.energyBinWidthGeV = histogram.energyCentersGeV[1] - histogram.energyCentersGeV[0];
    }
    if (histogram.thetaCentersRad.size() >= 2) {
        histogram.thetaBinWidthRad = histogram.thetaCentersRad[1] - histogram.thetaCentersRad[0];
    }
    histogram.totalWeight = 0.0;
    for (const auto& row : histogram.densityPerGeVRad) {
        for (double value : row) {
            histogram.totalWeight += value * histogram.energyBinWidthGeV * histogram.thetaBinWidthRad;
        }
    }
    return histogram;
}
...
inline double histogramL1Distance(const Histogram& lhs,
                                  const Histogram& rhs) {
...
inline double jointHistogramL1Distance(const JointHistogram& lhs,
                                       const JointHistogram& rhs) {
    if (lhs.energyCentersGeV.size() != rhs.energyCentersGeV.size()
        || lhs.thetaCentersRad.size() != rhs.thetaCentersRad.size()) {
        throw std::runtime_error("LinearBreitWheelerBenchmark: joint histogram sizes do not match.");
    }
    double distance = 0.0;
