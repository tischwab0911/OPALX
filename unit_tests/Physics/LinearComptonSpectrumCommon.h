...
inline std::size_t jointHistogramIndex(const JointHistogram& histogram,
...
inline JointHistogram integrateLabJointSpectrum(const JointConfig& config) {
    ...
    if (histogram.totalWeight <= 0.0) {
        throw std::runtime_error("LinearComptonBenchmark: joint histogram total weight is not positive.");
    }
    ...
    for (std::size_t i = 0; i < histogram.densityPerGeVRad.size(); ++i) {
        histogram.densityPerGeVRad[i] = histogram.counts[i] / (histogram.totalWeight * cellArea);
    }

    return histogram;
}

inline JointHistogram sampleLabJointSpectrum(const JointConfig& config,
                                             std::size_t sampleCount,
                                             std::uint64_t streamIndex = 0) {
    JointHistogram histogram;
    histogram.energyBinWidthGeV = (config.energyMaxGeV - config.energyMinGeV)
    ...
    if (histogram.totalWeight <= 0.0) {
        throw std::runtime_error("LinearComptonBenchmark: sampled joint histogram total weight is not positive.");
    }
...
inline void writeJointCSV(const JointHistogram& histogram,
                          const std::filesystem::path& outputPath) {
    std::ofstream output(outputPath);
    if (!output) {
        throw std::runtime_error("LinearComptonBenchmark: unable to open joint output file "
                                 + outputPath.string());
    }
    ...
    for (std::size_t i = 0; i < histogram.energyCentersGeV.size(); ++i) {
        for (std::size_t j = 0; j < histogram.thetaCentersRad.size(); ++j) {
            ...
            output << histogram.energyCentersGeV[i] << ','
                   ...
                   << histogram.densityPerGeVRad[index] << ','
...
inline JointHistogram readJointCSV(const std::filesystem::path& inputPath) {
    ...
    std::ifstream input(inputPath);
    if (!input) {
        throw std::runtime_error("LinearComptonBenchmark: unable to open joint input file "
                                 + inputPath.string());
    }
    ...
    while (std::getline(input, line)) {
        ...
        rows.push_back(Row{fields[0], fields[1], fields[2], fields.size() >= 4 ? fields[3] : 0.0});
    }

    auto containsCenter = [](const std::vector<double>& values, double target) {
        for (double value : values) {
            if (std::abs(value - target) <= 1.0e-12 * std::max(1.0, std::max(std::abs(value), std::abs(target)))) {
                return true;
            }
        }
        return false;
    ...
    histogram.densityPerGeVRad.assign(histogram.energyCentersGeV.size() * histogram.thetaCentersRad.size(), 0.0);
    histogram.counts.assign(histogram.energyCentersGeV.size() * histogram.thetaCentersRad.size(), 0.0);

    auto findIndex = [](const std::vector<double>& values, double target) {
        for (std::size_t i = 0; i < values.size(); ++i) {
            if (std::abs(values[i] - target) <= 1.0e-12 * std::max(1.0, std::max(std::abs(values[i]), std::abs(target)))) {
                return i;
            }
        }
        throw std::runtime_error("LinearComptonBenchmark: unable to locate joint histogram center.");
    };
    ...
    for (std::size_t i = 0; i < histogram.energyCentersGeV.size(); ++i) {
        for (std::size_t j = 0; j < histogram.thetaCentersRad.size(); ++j) {
            mean += histogram.energyCentersGeV[i]
                * histogram.densityPerGeVRad[jointHistogramIndex(histogram, i, j)] * cellArea;
        }
    }
    return mean;
}
