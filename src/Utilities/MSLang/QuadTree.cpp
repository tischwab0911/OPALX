...
namespace mslang {
    QuadTree::QuadTree(const QuadTree& right)
        : level_m(right.level_m),
          objects_m(right.objects_m.begin(), right.objects_m.end()),
          bb_m(right.bb_m),
          nodes_m() {
    ...
    void QuadTree::operator=(const QuadTree& right) {
        level_m = right.level_m;
        objects_m.insert(objects_m.end(), right.objects_m.begin(), right.objects_m.end());
        bb_m = right.bb_m;
    ...
    void QuadTree::transferIfInside(std::list<std::shared_ptr<Base> >& objs) {
        ...
        objs.remove_if([](const std::shared_ptr<Base> obj) {
            return !obj;
        });
    }
