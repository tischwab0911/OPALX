...
namespace mslang {
    void Ellipse::print(int indentwidth) {
        ...
        std::cout << indent << "ellipse, \n"
                  ...
                  << indent2 << "angle: " << angle << "\n"
                  << indent2 << std::setw(14) << trafo_m(0, 0) << std::setw(14) << trafo_m(0, 1)
                  << std::setw(14) << trafo_m(0, 2) << "\n"
                  << indent2 << std::setw(14) << trafo_m(1, 0) << std::setw(14) << trafo_m(1, 1)
                  << std::setw(14) << trafo_m(1, 2) << "\n"
                  << indent2 << std::setw(14) << trafo_m(2, 0) << std::setw(14) << trafo_m(2, 1)
                  << std::setw(14) << trafo_m(2, 2) << std::endl;
    }
    ...
    void Ellipse::writeGnuplot(std::ofstream& out) const {
        ...
        for (unsigned int i = 0; i < N; ++i, phi += dp) {
            ...
            pt[0] = std::copysign(
                    sqrt(std::pow(height_m * width_m * 0.25, 2)
                         / (std::pow(height_m * 0.5, 2) + std::pow(width_m * 0.5 * tan(phi), 2))),
                    cos(phi));
            pt[1] = pt[0] * tan(phi);
            pt    = trafo_m.transformFrom(pt);

            out << std::setw(colwidth) << pt[0] << std::setw(colwidth) << pt[1] << std::endl;
        }
        out << std::endl;
    ...
    void Ellipse::computeBoundingBox() {
        ...
        double halfwidth = 0.5 * (M11 * width_m * cos(t) + M12 * height_m * sin(t));
        llc[0]           = center[0] - std::abs(halfwidth);
        urc[0]           = center[0] + std::abs(halfwidth);

        t = atan2(height_m * M22, width_m * M21);

        double halfheight = 0.5 * (M21 * width_m * cos(t) + M22 * height_m * sin(t));
    ...
    bool Ellipse::parse_detail(iterator& it, const iterator& end, Function* fun) {
        ...
        if (elps->width_m < 0.0) {
            std::cout << "Ellipse: a negative width provided '" << arguments.get(0) << " = "
                      << elps->width_m << "'" << std::endl;
            return false;
        }
        if (elps->height_m < 0.0) {
            std::cout << "Ellipse: a negative height provided '" << arguments.get(1) << " = "
                      << elps->height_m << "'" << std::endl;
            return false;
        }
