...
namespace mslang {
    void Triangle::print(int indentwidth) {
        ...
        std::cout << indent << indentbase << "origin: " << origin[0] << ", " << origin[1] << ",\n"
                  ...
                  << indent << indentbase << trafo_m(0, 0) << "\t" << trafo_m(0, 1) << "\t"
                  << trafo_m(0, 2) << "\n"
                  << indent << indentbase << trafo_m(1, 0) << "\t" << trafo_m(1, 1) << "\t"
                  << trafo_m(1, 2) << "\n"
                  << indent << indentbase << trafo_m(2, 0) << "\t" << trafo_m(2, 1) << "\t"
    ...
    void Triangle::writeGnuplot(std::ofstream& out) const {
        ...
        for (unsigned int i = 0; i < 4u; ++i) {
            ...
            out << std::setw(width) << corner[0] << std::setw(width) << corner[1] << std::endl;
        }
        out << std::endl;
