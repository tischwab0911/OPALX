...
inline unsigned int PortableBitmapReader::getWidth() const { return width_m; }

inline unsigned int PortableBitmapReader::getHeight() const { return height_m; }

inline bool PortableBitmapReader::isBlack(unsigned int i, unsigned int j) const {
    return pixels_m[getIdx(i, j)];
}

inline std::vector<bool> PortableBitmapReader::getPixels() const { return pixels_m; }

inline unsigned int PortableBitmapReader::getIdx(unsigned int h, unsigned int w) const {
    if (h >= height_m || w >= width_m)
        throw OpalException("PortableBitmapReader::getIdx", "Pixel number out of bounds");
    return h * width_m + w;
}
