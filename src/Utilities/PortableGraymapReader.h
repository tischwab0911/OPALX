...
inline unsigned int PortableGraymapReader::getWidth() const { return width_m; }

inline unsigned int PortableGraymapReader::getHeight() const { return height_m; }

inline unsigned short PortableGraymapReader::getPixel(unsigned int i, unsigned int j) const {
    return pixels_m[getIdx(i, j)];
}

inline std::vector<unsigned short> PortableGraymapReader::getPixels() const { return pixels_m; }

inline unsigned int PortableGraymapReader::getIdx(unsigned int h, unsigned int w) const {
    if (h >= height_m || w >= width_m)
        throw OpalException("PortableGraymapReader::getIdx", "Pixel number out of bounds");
    return h * width_m + w;
}
