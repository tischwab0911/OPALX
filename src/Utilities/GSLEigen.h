...
inline void gsl_eigen_nonsymm_free(gsl_eigen_nonsymm_workspace* w) { delete w; }
...
/// \param w Input: workspace to release (can be null).
inline void gsl_eigen_nonsymmv_free(gsl_eigen_nonsymmv_workspace* w) { delete w; }
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        for (size_t row = 0; row < n; ++row) {
            for (size_t col = 0; col < n; ++col) {
                if (row == col) {
                    gsl_complex diag =
                        gsl_complex_sub(gsl_complex(*gsl_matrix_ptr(A, row, col), 0.0), lambda);
                    *gsl_matrix_complex_ptr(A_minus_lambda, row, col) = diag;
                } else {
                    *gsl_matrix_complex_ptr(A_minus_lambda, row, col) =
                        gsl_complex(*gsl_matrix_ptr(A, row, col), 0.0);
                }
            }
        }
        ...
        for (size_t j = 0; j < n; ++j) {
            *gsl_matrix_complex_ptr(evec, j, i) =
                (j == n - 1) ? gsl_complex(1.0, 0.0) : gsl_complex(0.0, 0.0);
        }
