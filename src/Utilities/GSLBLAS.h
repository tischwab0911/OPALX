    ...
    for (size_t i = 0; i < m; ++i) {
        for (size_t j = 0; j < n; ++j) {
            ...
            for (size_t l = 0; l < k; ++l) {
                double a_val = (TransA == CblasNoTrans) ? 
                    *gsl_matrix_ptr(A, i, l) : *gsl_matrix_ptr(A, l, i);
                double b_val = (TransB == CblasNoTrans) ? 
                    *gsl_matrix_ptr(B, l, j) : *gsl_matrix_ptr(B, j, l);
                sum += a_val * b_val;
            }
...
inline void gsl_blas_zgemm(CBLAS_TRANSPOSE TransA, CBLAS_TRANSPOSE TransB,
    ...
    if (beta.dat[0] != 1.0 || beta.dat[1] != 0.0) {
        for (size_t i = 0; i < m; ++i) {
            for (size_t j = 0; j < n; ++j) {
                *gsl_matrix_complex_ptr(C, i, j) = gsl_complex_mul(*gsl_matrix_complex_ptr(C, i, j), beta);
            }
        }
    }
    ...
    for (size_t i = 0; i < m; ++i) {
        for (size_t j = 0; j < n; ++j) {
            ...
            for (size_t l = 0; l < k; ++l) {
                gsl_complex a_val = (TransA == CblasNoTrans) ? 
                    *gsl_matrix_complex_ptr(A, i, l) : *gsl_matrix_complex_ptr(A, l, i);
                if (TransA == CblasConjTrans) {
                    a_val = gsl_complex_conjugate(a_val);
                }
                gsl_complex b_val = (TransB == CblasNoTrans) ? 
                    *gsl_matrix_complex_ptr(B, l, j) : *gsl_matrix_complex_ptr(B, j, l);
                if (TransB == CblasConjTrans) {
                    b_val = gsl_complex_conjugate(b_val);
                }
                sum = gsl_complex_add(sum, gsl_complex_mul(a_val, b_val));
            }
            *gsl_matrix_complex_ptr(C, i, j) = gsl_complex_add(*gsl_matrix_complex_ptr(C, i, j), 
                                                               gsl_complex_mul(alpha, sum));
        }
    }
}
...
inline void gsl_blas_zgemv(CBLAS_TRANSPOSE TransA,
    ...
    if (TransA == CblasNoTrans) {
        ...
        for (size_t i = 0; i < m; ++i) {
            gsl_complex sum = gsl_complex(0.0, 0.0);
            for (size_t j = 0; j < n; ++j) {
                sum = gsl_complex_add(sum, gsl_complex_mul(*gsl_matrix_complex_ptr(A, i, j), 
                                                           *gsl_vector_complex_ptr(x, j)));
            }
            *gsl_vector_complex_ptr(y, i) = gsl_complex_add(*gsl_vector_complex_ptr(y, i), 
                                                            gsl_complex_mul(alpha, sum));
        }
    } else {
        if (x->size != m || y->size != n) {
