...
public:
    ...
    double uniform() { return uniform_dist_(engine_); }
    ...
    void seed(unsigned long seed) { engine_.seed(seed); }
...
inline gsl_rng* gsl_rng_alloc(const gsl_rng_type* /* type */) { return new gsl_rng(); }

inline void gsl_rng_free(gsl_rng* rng) { delete rng; }

inline double gsl_rng_uniform(gsl_rng* rng) { return rng->uniform(); }

inline double gsl_ran_gaussian(gsl_rng* rng, double sigma) { return rng->gaussian(sigma); }
