...
Echo* Echo::clone(const std::string& name) { return new Echo(name, this); }

void Echo::execute() {
    if (ippl::Comm->rank() == 0) std::cerr << Attributes::getString(itsAttr[0]) << std::endl;
}

void Echo::parse(Statement& statement) { parseShortcut(statement); }
