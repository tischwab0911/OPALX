...
Attribute::Attribute() : base(nullptr), handler(), isDefault(true) {}

Attribute::Attribute(const Attribute& rhs)
    : base(rhs.base), handler(rhs.handler), isDefault(rhs.isDefault) {}

Attribute::Attribute(AttributeHandler* h, AttributeBase* b)
    : base(b), handler(h), isDefault(true) {}
...
AttributeBase& Attribute::getBase() const { return *base; }

bool Attribute::isBaseAllocated() const { return base != nullptr; }

AttributeHandler& Attribute::getHandler() const { return *handler; }

const std::string& Attribute::getHelp() const { return handler->getHelp(); }

std::string Attribute::getImage() const { return base->getImage(); }

const std::string& Attribute::getName() const { return handler->getName(); }

const std::string& Attribute::getType() const { return handler->getType(); }

bool Attribute::isDeferred() const { return handler->isDeferred(); }

void Attribute::setDeferred(bool flag) { handler->setDeferred(flag); }

bool Attribute::isExpression() const { return base->isExpression(); }

bool Attribute::isReadOnly() const { return handler->isReadOnly(); }

void Attribute::setReadOnly(bool flag) { handler->setReadOnly(flag); }
