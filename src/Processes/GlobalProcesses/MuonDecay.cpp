...
void MuonDecay::createDaughterParticles(
    ...
    Kokkos::parallel_for(
        "MuonDecay::createDaughters",
        static_cast<pc_size_type>(localDestroyNum),
        KOKKOS_LAMBDA(const pc_size_type j) {
            ...
            const double bg2 = parentMom[0] * parentMom[0]
                              + parentMom[1] * parentMom[1]
            ...
            if (beta2 > 0.0) {
                ...
                const double factor =
                    (gamma - 1.0) * betaDotP / beta2 + gamma * eDaughter;
