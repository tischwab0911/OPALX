...
namespace Expressions {
    ...
    template <class T, class U>
    inline ABinary<T, U>::ABinary(const ABinary& rhs)
    ...
    template <class T, class U>
    inline OArray<T>* ABinary<T, U>::clone() const {
        return new ABinary<T, U>(*this);
    }

    template <class T, class U>
    inline std::vector<T> ABinary<T, U>::evaluate() const {
        errno              = 0;
        ...
        if (op1.size() == op2.size()) {
            ...
            throw OpalException("ABinary::evaluate()", "Inconsistent array dimensions.");
        }
    ...
    template <class T, class U>
    inline void ABinary<T, U>::print(std::ostream& os, int precedence) const {
        if (fun.precedence >= 0) {
            // Binary operation.
