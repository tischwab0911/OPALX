...
namespace Expressions {
    ...
    template <class T>
    inline SNull<T>::SNull(const SNull<T>& rhs) : Scalar<T>(rhs), fun(rhs.fun) {}

    template <class T>
    inline SNull<T>::SNull(const TFunction0<T>& function) : fun(function) {}

    template <class T>
    inline SNull<T>::~SNull() {}

    template <class T>
    inline Scalar<T>* SNull<T>::clone() const {
        return new SNull<T>(*this);
    }

    template <class T>
    inline T SNull<T>::evaluate() const {
        errno    = 0;
    ...
    template <class T>
    inline Scalar<T>* SNull<T>::make(const TFunction0<T>& fun) {
        return new SNull<T>(fun);
    }

    template <class T>
    inline void SNull<T>::print(std::ostream& stream, int) const {
        stream << fun.name << "()";
    }
