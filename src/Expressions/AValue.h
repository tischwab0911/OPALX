...
namespace Expressions {
    ...
    template <class T>
    class AValue : public AttributeBase {
    public:
        /// Default constructor.
    ...
    template <class T>
    AValue<T>::AValue() : value() {}

    template <class T>
    AValue<T>::AValue(const AValue<T>& rhs) : value(rhs.value) {}

    template <class T>
    AValue<T>::AValue(const std::vector<T>& val) : value(val) {}
    ...
    template <>
    inline void AValue<bool>::print(std::ostream& os) const {
        os << '{';
    ...
    template <>
    inline void AValue<double>::print(std::ostream& os) const {
    ...
    template <>
    inline void AValue<std::string>::print(std::ostream& os) const {
        os << '{';
    ...
    template <>
    inline void AValue<std::list<Token> >::print(std::ostream& os) const {
        os << '{';
        std::vector<std::list<Token> >::const_iterator i = value.begin();

        while (i != value.end()) {
            for (std::list<Token>::const_iterator j = i->begin(); j != i->end(); ++j) {
                os << *j;
            }
