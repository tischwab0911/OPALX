...
namespace Expressions {
    ...
    template <class T>
    inline ASUnary<T>::ASUnary(const ASUnary<T>& rhs)
    ...
    template <class T>
    inline Scalar<T>* ASUnary<T>::clone() const {
        return new ASUnary<T>(*this);
    }

    template <class T>
    inline T ASUnary<T>::evaluate() const {
        errno                    = 0;
    ...
    template <class T>
    inline void ASUnary<T>::print(std::ostream& os, int precedence) const {
        if (fun.precedence >= 0) {
