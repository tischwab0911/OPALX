...
//   This string is intended for storing valid OPALX expressions,
...
StringStream::StringStream(const std::string& str)
    : TokenStream("expression"), line_m(str + '\n'), currentChar_m(0) {}
...
Token StringStream::readToken() {
    ...
    if (ch == '"' || ch == '\'') {
        ...
        // Word token.
        return readWord();
    } else if (isdigit(ch) || (ch == '.' && isdigit(line_m[currentChar_m + 1]))) {
        // Numeric token.
        return readNumber();
    } else {
        // Delimiter tokens.
...
Token StringStream::readNumber() {
    ...
    if (eflag) {
        return Token("string", 1, Token::IS_ERROR, "Invalid numeric token \"" + lexeme + "\".");
    } else {
    ...
    if (line_m[currentChar_m] == '"' || line_m[currentChar_m] == '\'') {
        ...
        while (true) {
            if (currentChar_m >= line_m.length()) {
                throw FormatError("StringStream::readString()", "String not terminated.");
            }
