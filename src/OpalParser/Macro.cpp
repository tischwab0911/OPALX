...
bool Macro::shouldTrace() const { return false; }

bool Macro::shouldUpdate() const { return false; }
...
void Macro::parseFormals(Statement& stat) {
    ...
    if (!stat.delimiter(')')) {
        do {
            std::string form = Expressions::parseString(stat, "Expected formal argument name.");
