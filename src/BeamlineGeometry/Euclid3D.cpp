...
Euclid3D::Euclid3D(double x, double y, double z, double vx, double vy, double vz)
...
bool Euclid3D::operator==(const Euclid3D& rhs) const { return (V == rhs.V) && (R == rhs.R); }

bool Euclid3D::operator!=(const Euclid3D& rhs) const { return (V != rhs.V) || (R != rhs.R); }

void Euclid3D::getAll(double& x, double& y, double& z, double& vx, double& vy, double& vz) const {
    V.getComponents(x, y, z);
    R.getAxis(vx, vy, vz);
}

const Vector3D& Euclid3D::getVector() const { return V; }

const Rotation3D& Euclid3D::getRotation() const { return R; }

void Euclid3D::setX(double x) { V.setX(x); }

void Euclid3D::setY(double y) { V.setY(y); }

void Euclid3D::setZ(double z) { V.setZ(z); }
...
const Euclid3D& Euclid3D::operator*=(const Euclid3D& rhs) { return dotBy(rhs); }
...
bool Euclid3D::isPureTranslation() const { return R.isIdentity(); }

bool Euclid3D::isPureXRotation() const { return V.isZero() && R.isPureXRotation(); }

bool Euclid3D::isPureYRotation() const { return V.isZero() && R.isPureYRotation(); }

bool Euclid3D::isPureZRotation() const { return V.isZero() && R.isPureZRotation(); }

Euclid3D Euclid3D::identity() { return Euclid3D(); }
