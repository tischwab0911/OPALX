...
DirichletPlaneWriter::DirichletPlaneWriter(const std::string& outputDirectory)
    ...
    if (ec) {
        throw OpalException(
                "DirichletPlaneWriter::DirichletPlaneWriter", "Failed to create output directory \""
                                                                      + outputDirectory_m.string()
                                                                      + "\": " + ec.message());
    }

    m << level4 << "Dirichlet-plane diagnostics output directory: \"" << outputDirectory_m.string()
      << "\"." << endl;
}

void DirichletPlaneWriter::writePlane(
        long long step, double time, double zPlane, const std::vector<double>& xCoords,
        const std::vector<double>& yCoords, const std::vector<double>& phiValues, std::size_t nx,
        std::size_t ny, const std::string& solveTag) {
    ...
    fileName << "phi_dirichlet_" << safeTag << "_step-" << std::setfill('0') << std::setw(8) << step
             << ".dat";
    ...
    out << "# step=" << step << " time=" << time << " [s] zPlane=" << zPlane
        << " [m] tag=" << safeTag << "\n";
    out << "# nx=" << nx << " ny=" << ny << "\n";
    ...
    for (std::size_t i = 0; i < nx; ++i) {
        for (std::size_t j = 0; j < ny; ++j) {
            ...
            out << std::setw(6) << i << std::setw(6) << j << " " << std::scientific << xCoords[i]
                << " " << yCoords[j] << " " << phiValues[idx] << "\n";
        }
    }
}
