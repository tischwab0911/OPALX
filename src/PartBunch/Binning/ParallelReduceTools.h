...
namespace ParticleBinning {
     ...
     *
     * Member Functions:
     * - **ArrayReduction()**: Default constructor that initializes all elements of the array to
     * zero.
     * - **ArrayReduction(const ArrayReduction& rhs)**: Copy constructor that performs a deep copy
     * of the array.
     * - **ArrayReduction& operator=(const ArrayReduction& rhs)**: Assignment operator that performs
     * a deep copy of the array.
     * - **ArrayReduction& operator+=(const ArrayReduction& src)**: Element-wise addition operator
     * that adds the elements of another `ArrayReduction` instance to the current instance.
     */
    ...
    */
    ...
    /**
     * @brief Primary template for generating std::variant types containing ArrayReduction
     * specializations.
     *
    ...
    /**
     * @brief Specialized template that expands `std::integer_sequence` to create a variant of
     * `ArrayReduction` types.
     *
    ...
    /**
     * @brief Type alias for a `std::variant` containing all possible `ArrayReduction` types up to
     * `maxArrSize`.
     *
     * This alias creates a variant that can hold `ArrayReduction` objects with sizes from `1` to
     * `maxArrSize`. It uses `ReductionVariantHelper` with `std::make_integer_sequence` to generate
     * all variants type at compile time.
     *
     * @tparam SizeType The type used for array elements in the reduction operations.
     * @tparam IndexType The type used for array indexing and size specification.
     */
    template <typename SizeType, typename IndexType>
    using ReductionVariant = typename ReductionVariantHelper<
            SizeType, IndexType,
            std::make_integer_sequence<IndexType, maxArrSize<IndexType>>>::type;

    /**
     * @brief Recursive helper function to create `ArrayReduction` objects with compile-time size
     * matching.
     *
     * This function uses compile-time recursion to find and instantiate the correct
     * `ArrayReduction` type based on the requested `binCount`. It recursively increments the
     * template parameter `N` until it matches `binCount`, then returns the appropriate
     * `ArrayReduction` instance.
     *
     * @tparam SizeType The type used for array elements in the reduction operations.
     * @tparam IndexType The type used for array indexing.
     ...
     *
     * @return ReductionVariant containing the `ArrayReduction` object with size matching
     * `binCount`.
     *
     * @throws `std::out_of_range` If `binCount` exceeds `maxArrSize`.
     *
     * @note This function should not be called directly. Use `createReductionObject()` instead and
     * use `std::visit` to access the reducer.
     */
    template <typename SizeType, typename IndexType, IndexType N>
     ...
     * @tparam SizeType The type used for array elements in the reduction operations.
     * @tparam IndexType The type used for array indexing.
     * @param binCount The desired number of bins for the reduction array (must be $\leq$
     * `maxArrSize`).
     *
    ...
    /**
     * @brief Host-only array reduction structure for dynamic array sizes in
     * `Kokkos::parallel_reduce`.
     *
     * This structure provides array-based reduction functionality specifically only for host
     * execution. Unlike `ArrayReduction` which uses compile-time fixed sizes, HostArrayReduction
     * uses dynamically allocated arrays with runtime-determined sizes. This approach offers better
     * performance than team-based reductions on host systems (low concurrency) while maintaining
     * flexibility for arbitrary bin counts (way more efficient than pure atomics).
     *
     * @tparam SizeType The type used for the elements of the array.
     * @tparam IndexType The type used for indexing and array size specification.
     *
     * @note This structure is conditionally compiled and only available when
     * OPALX_DEVICE_COMPILATION is not defined.
     * @note All instances of this class share the same array size through the static binCountStatic
     * member.
     *
    ...
    struct HostArrayReduction {
        ...
        /**
         * @brief Static variable defining the array size for all instances of this template
         * specialization.
         *
         * @note This value is set outside when the classed is used, typically before any instances
         * are created.
        ...
        HostArrayReduction() {
            ...
            for (IndexType i = 0; i < binCountStatic; i++) {
                the_array[i] = 0;
            }
        }
        ...
        /**
         * @brief Element-wise addition operator that adds the elements of another
         * `HostArrayReduction` instance.
         *
        ...
        HostArrayReduction& operator+=(const HostArrayReduction& src) {
            ...
            Kokkos::abort(
                    "Error: HostArrayReduction is not supported on device backends "
     ...
     *
     * @note This must be defined outside the class template for proper static member
     * initialization.
     * @note The value should be set appropriately before creating any `HostArrayReduction`
     * instances.
...
namespace Kokkos {
    ...
    struct reduction_identity<ParticleBinning::ArrayReduction<SizeType, IndexType, N>> {
        KOKKOS_FORCEINLINE_FUNCTION static ParticleBinning::ArrayReduction<SizeType, IndexType, N>
        sum() {
     ...
     * @see reduction_identity<ParticleBinning::ArrayReduction> for detailed explanation of
     * reduction identities.
     */
    template <typename SizeType, typename IndexType>
    struct reduction_identity<ParticleBinning::HostArrayReduction<SizeType, IndexType>> {
        KOKKOS_FORCEINLINE_FUNCTION static ParticleBinning::HostArrayReduction<SizeType, IndexType>
        sum() {
