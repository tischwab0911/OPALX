...
namespace ParticleBinning {
     ...
     *                       - `ParallelReduce`, `TeamBased`, or `HostOnly` to force a specific
     * mode.
    ...
    HistoReductionMode determineHistoReductionMode(
        ...
        // standard!
        if (modePreference == HistoReductionMode::Standard) {  //  || modePreference ==
                                                               //  HistoReductionMode::HostOnly
            // if (modePreference == HistoReductionMode::HostOnly) {
            //     std::cerr << "Warning: HostOnly mode is not supported on CUDA! Switching to
            //     Standard mode." << std::endl;
            // }
     ...
     * ```
     *
     * @note Using a shared pointer does not work, since these are managed on host only! Using a
     * simple passed Kokkos::View also does not work, since create() might be called which calls
     * Kokkos::realloc(), which is a problem, since the copied view does not know the reallocated
     * data. Therefore, the view is updated with `updateDataArr`!
     *
