...
namespace ParticleBinning {
    ...
    value_type Histogram<size_type, bin_index_type, value_type, UseDualView, Properties...>::
        ...
        if (sumCount == 0) {
            ...
            err << "Error in adaptiveBinningCostFunction: "
                << "sumCount = " << sumCount << ", sumWidth = " << sumWidth
        ...
        value_type sparse_penalty =
                (sumCountNorm < desiredWidth_m) ? desiredWidth_m / sumCountNorm : 0.0;

        // Sanity checks
        if (sumCountNorm <= 0 || sumCountNorm > 1) {
            Inform err("mergeBins");
            err << "Error in adaptiveBinningCostFunction: "
                << "sumCountNorm = " << sumCountNorm << ", sumWidth = " << sumWidth
                << ", totalNumParticles = " << totalNumParticles << endl;
            ippl::Comm->abort();
        }
    ...
    template <
            ...
            class... Properties>
    Histogram<
            size_type, bin_index_type, value_type, UseDualView,
            Properties...>::hindex_transform_type
    Histogram<size_type, bin_index_type, value_type, UseDualView, Properties...>::mergeBins() {
        ...
        */
        if constexpr (!std::is_same<typename hview_type::memory_space, Kokkos::HostSpace>::value) {
            m << level4
              << "This does not work if the histogram is not saved in a DualView, since it needs "
        ...
        if (n < 2) {
            // Should not happen, since this function is to be called after generating a very fine
            // histogram, e.g. 128 bins
        ...
        const size_type totalNumParticles =
                prefixCount(n);  // Last value in prefixCount is the total number of particles
        ...
        // Kokkos::View<value_type*, Kokkos::HostSpace> dpMoment("dpMoment", n+1); // Store
        // cumulative moments --> allow first order moment error estimation
        ...
        for (bin_index_type k = 1; k <= n; ++k) {
            ...
            for (bin_index_type i = 0; i < k; ++i) {
                ...
                if (sumCount > 0) {
                    // Division by 0 if called with sumCount=0. Merge anyways --> set cost to a
        ...
        if (totalCost >= largeVal) {
            ...
            std::cerr << "Warning: no feasible merges found. Setting cost=0, no merges."
                      << std::endl;
        ...
        // m << "Merged bins (based on minimal cost partition): " << mergedBinsCount << ". Minimal
        ...
        // Also generate a lookup table that maps the old bin index to the new bin index (for
        // "rebin")
        hindex_transform_type oldToNewBinsView("oldToNewBinsView", n);
        for (size_type j = 0; j < mergedBinsCount; ++j) {
        ...
        m << level3 << "Re-binned from " << n << " bins down to " << numBins_m
          << " bins. Total deviation cost = " << totalCost << endl;
