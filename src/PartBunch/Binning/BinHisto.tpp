...
namespace ParticleBinning {
    ...
    template <
        ...
        class... Properties>
    Histogram<
        size_type, bin_index_type, value_type, UseDualView, Properties...>::hindex_transform_type
    Histogram<size_type, bin_index_type, value_type, UseDualView, Properties...>::mergeBins() {
        ...
        */
        if constexpr (!std::is_same<typename hview_type::memory_space, Kokkos::HostSpace>::value) {
            m << "This does not work if the histogram is not saved in a DualView, since it needs "
        ...
        m << "Re-binned from " << n << " bins down to " << numBins_m
