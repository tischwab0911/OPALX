...
namespace ParticleBinning {
    ...
    void AdaptBins<BunchType, BinningSelector>::initLimits() {
        ...
        if (nlocal <= 0) {
            msg << "Particles in the bunch = " << nlocal << ". Overwriting limits manually."
                << endl;
        ...
        msg << "Initialized limits. Min: " << xMin_m << ", max: " << xMax_m
    ...
    void AdaptBins<BunchType, BinningSelector>::assignBinsToParticles() {
        ...
        Kokkos::parallel_for(
            "assignParticleBinsConst", bunch_m->getLocalNum(), KOKKOS_LAMBDA(const size_type& i) {
    ...
    void AdaptBins<BunchType, BinningSelector>::initLocalHisto(HistoReductionMode modePreference) {
        ...
        if (mode == HistoReductionMode::HostOnly) {
            ...
            std::visit(
                [&](auto& reducer_arr) {
                    msg << "Starting parallel_reduce, array size = "
    ...
    void AdaptBins<BunchType, BinningSelector>::initGlobalHistogram() {
        ...
        hview_type_g globalBinHistoHost =
    ...
    VField_t<T, Dim>& AdaptBins<BunchType, BinningSelector>::LTrans(
        ...
        Kokkos::parallel_reduce(
            "CalculateGammaFactor", getBinIterationPolicy(currentBin),
        ...
        // Next apply the transformation --> do it manually, since fc->E*gamma does not exist in
        // IPPL...
        ippl::parallel_for(
