...
void FieldSolver<double, 3>::dumpScalField(std::string what) {
    ...
    Field_t<3>* field =
        (this->getStype() == "CG" && Util::toUpper(what) == "PHI")
            ? this->getPhi()
            : this->getRho();  // both rho and phi are in the same variable (in place computation)
...
void FieldSolver<double, 3>::runSolver(bool force_skip_field_dump) {
    ...
    for now.
    */
    m << "Running solver with type: " << this->getStype()
