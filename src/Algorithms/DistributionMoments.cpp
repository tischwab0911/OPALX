...
void DistributionMoments::computeMoments(ippl::ParticleAttrib<Vector_t<double,3>>::view_type  Rview,
    ...
    */
    ...
    // computeMeans + kinetic-energy moments (mass-aware via Mview)
    const int Dim = 3;
    double loc_centroid[2 * Dim] = {};
    double centroid[2 * Dim]     = {};
    double loc_Ekin, loc_gamma, loc_gammaz, gammaz;
    ...
    Kokkos::parallel_reduce(
        ...
        KOKKOS_LAMBDA(const int k, double& ekin, double& gamma, double& gammaz) {
            ...
            const double mGeV = rescaleToReference ? referenceMassGeV : Mview(k);
            ekin0              = (gamma0 - 1.0) * mGeV * Units::GeV2MeV; // output in MeV
    ...
    double mekin                      = meanKineticEnergy_m;
    double loc_std_mekin              = 0.0;
    const bool rescaleToReferenceStd = rescaleToReference_m;
    const double referenceMassGeVStd = referenceMassGeV_m;

    // compute non-central moments (positions/p-moment only)
    for (unsigned i = 0; i < 2 * Dim; ++i) {
        Kokkos::parallel_reduce(
            "calc moments of particle distr.", Nlocal,
            KOKKOS_LAMBDA(const int k, double& mom0, double& mom1, double& mom2,
            ...
            Kokkos::Sum<double>(loc_moment_ncent[i][0]), Kokkos::Sum<double>(loc_moment_ncent[i][1]),
            Kokkos::Sum<double>(loc_moment_ncent[i][2]), Kokkos::Sum<double>(loc_moment_ncent[i][3]),
            Kokkos::Sum<double>(loc_moment_ncent[i][4]), Kokkos::Sum<double>(loc_moment_ncent[i][5]));
        Kokkos::fence();
    }
    ippl::Comm->barrier();

    // std kinetic energy (mass-aware)
    Kokkos::parallel_reduce(
        "calc moments of particle distr.", Nlocal,
        KOKKOS_LAMBDA(const int k, double& ekin) {
            ...
            const double mGeV = rescaleToReferenceStd ? referenceMassGeVStd : Mview(k);
            ekin0              = (gamma0 - 1.0) * mGeV * Units::GeV2MeV; // output in MeV
    ...
    ippl::Comm->allreduce(&loc_moment_ncent[0][0], &moment_ncent[0][0], 2 * Dim * 2 * Dim, std::plus<double>());
