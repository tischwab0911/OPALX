...
template <class T>
IndirectChannel<T>::IndirectChannel(T& object, double (T::*get)() const, void (T::*set)(double))
    : itsObject(object), getF(get), setF(set) {}

template <class T>
IndirectChannel<T>::IndirectChannel(const IndirectChannel& rhs)
    : Channel(), itsObject(rhs.itsObject), getF(rhs.getF), setF(rhs.setF) {}
...
template <class T>
inline bool IndirectChannel<T>::isSettable() const {
    return (setF != 0);
}
