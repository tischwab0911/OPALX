...
void FlatTop::generateParticles(size_t& numberOfParticles, Vector_t<double, 3> nr) {
    ...
    if (emitting_m) {
        ...
        // pc_m->updateTotal function instead?
...
double FlatTop::countEnteringParticlesPerRank(double t0, double tf) {
    ...
    if (totalNew > 0) {
        if (!withDomainDecomp_m) {
            ...
            // emission time find min/max of particle positions for [t,t+dt]
            ...
            if (prmax[0] >= locrmin[0] && prmin[0] <= locrmax[0] && prmax[1] >= locrmin[1]
                && prmin[1] <= locrmax[1] && prmax[2] >= locrmin[2] && prmin[2] <= locrmax[2]) {
